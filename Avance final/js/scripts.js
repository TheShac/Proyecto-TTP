...
function verCarrito() {
    ...
    items.forEach(item => {
        ...
        const priceText = item.querySelector('p').textContent.trim(); // Obtener el texto del precio y limpiar espacios
        const price = parseInt(priceText.replace('$', '').replace(' CLP', '').replace('.', '')); // Remover caracteres y convertir a número
